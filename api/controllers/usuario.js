...
const usuarioGet = async(req, res = response)=>{
    const{documento}= req.query // desestructuracion obtiene lo que se manda del navegador
...
const usuarioPost = async(req,res = response) => {
    ...
    try {
        ...
        if (error.name === 'ValidationError') {
          ...
          mensaje = 'Hubo un error , verificar datos';
        } else {
...
const usuarioPut= async(req, res= response)=>{
    ...
    const{documento, nombre, nota1, nota2, nota3, observaciones}=req.body
    let mensaje=''
   ...
   // el campo 1 es con el cual se va hacer la busqueda los demas son los campos que se va a modificar ejem nombre:nombre
    ...
    try{
        const usuario= await Usuario.findOneAndUpdate({documento:documento}, {nombre:nombre, nota1:nota1, nota2:nota2, nota3: nota3, observaciones:observaciones})
